...
def iterate_docs():
    ...
    for file in files:
        path = Path(file)
        if not os.path.exists(f'tmp/{str(path.parent.name)}'): ## Test case file directory could expand e.g _cci2_ko
            os.mkdir(f'tmp/{str(path.parent.name)}')

              # location to store new versions of each file
        tmpPath = f'tmp/{str(path.parent.name)}/{str(Path(path).stem)}{str(Path(path).suffix)}'

        f = open(file, "r")

        content = f.read()

        if matchFrontmatter := re.search(r'^---[\s\S]+?---', content):
            if matchFrontmatter[0].find("version:") != -1: # check if version tags exist, if no version tags do nothing
                ...
                if removeFirstnLast.find(":") != -1: # check if any other meta data after version
                    for line in removeFirstnLast.splitlines(): # grab only the children after version
                        if line[0] in ['-', ' ']:     # (---) been removed do not have to worry about it as edge case
                            lines += line + '\n'
                        else:
                            break
